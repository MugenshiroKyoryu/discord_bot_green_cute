...
หลายซีซั่นถูกยัดไว้ในสตริงเดียวคั่นด้วย "/" (ช่องว่างรอบตัวคั่นไม่แน่นอน)
และติดป้ายซีซั่นไว้ท้ายรายการในวงเล็บ ('Vol 1, Chap 0 (S1) / Vol 8, Chap 61 (S2)')
...
# ต้นทางวางช่องว่างรอบตัวคั่นไม่สม่ำเสมอ - เจอทั้ง ' / ' และ ' /' ติดกัน
# ('Vol 1, Chap 1 (S1) /Vol 5, Chap 34 (S2)') ยึด ' / ' ตายตัวจะหาตัวคั่นไม่เจอ
# ทั้งสตริง เหลือรายการเดียวยาว ๆ แล้วป้ายท้ายสุดจะกลืนทุกซีซั่นก่อนหน้าไป
#
# ตัดที่ "/" เปล่า ๆ ก็ไม่ได้ จะไปหั่นเลขตอนอย่าง 'Vol 1, Chap 1/2' ขาดกลาง
# จึงบังคับว่าต้องมีช่องว่างอย่างน้อยหนึ่งฝั่ง หรือตามหลังวงเล็บปิดของป้าย
...
def _tokens(text: str | None) -> list[tuple[str | None, str]]:
    ...
    for part in _SEPARATOR.split(cleaned):
        part = part.strip()
        if not part:
            continue
