...
| อนิเมะ | `anime.start`, `anime.end` | จัดรูปด้วย `format_anime_chapters()` — ซ่อนฟิลด์เมื่อไม่มีข้อมูลอนิเมะ ดูหัวข้อถัดไป |
...
คั่นด้วย `/` (ช่องว่างรอบตัวคั่นไม่แน่นอน) และติดป้ายซีซั่นไว้ท้ายรายการในวงเล็บ
ข้อมูลจึงถูกจัดกลุ่มคนละแกนกับที่คนอ่านต้องการ —
...
ต้นทางใช้วงเล็บท้ายค่าสองความหมาย — ป้ายซีซั่น กับ **หมายเหตุของตอน** เรื่องที่มีตอนถูกข้าม
(`Kaoru Hana wa Rin to Saku`: `Vol 6, Chap 40 (Skips Chap 32, Partly skips Chap 23-24, 27)`)
หมายเหตุจึงอยู่ติดกับฝั่งที่มันอธิบาย ไม่ใช่ขึ้นหน้าบรรทัดแทนชื่อซีซั่น:
...
| แยกรายการ | ตัดที่ `/` เมื่อมีช่องว่างอย่างน้อยหนึ่งฝั่ง หรือตามหลังวงเล็บปิดของป้าย — ต้นทางเขียนช่องว่างไม่สม่ำเสมอ (`(S1) /Vol 5`) แต่ตัดที่ `/` เปล่า ๆ ก็ไม่ได้ เลขตอนอย่าง `Chap 1/2` จะขาดกลาง |
| อ่านป้าย | ดึงป้ายจากวงเล็บท้ายรายการ รับรูปแบบที่ต้นทางใช้จริงไม่ใช่แค่ `S1` `S2` (`Season 2` `OVA` `ONA` `Movie` `Special` `Part 2` `Live Action`) และไล่ถอดได้หลายชั้นเมื่อป้ายกับหมายเหตุติดกัน (`(S1) (Skips Chap 32)`) |
| แยกหมายเหตุ | วงเล็บที่ไม่ใช่ป้าย (`(Skips Chap 32, Partly skips Chap 23-24, 27)`) ถือเป็นหมายเหตุของตอน คงไว้ติดกับตอนของฝั่งนั้น — เอาไปวางตำแหน่งป้ายจะกลายเป็นชื่อซีซั่นหน้าบรรทัด |
